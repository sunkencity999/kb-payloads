...
if "$HERE/test_beacon_term.sh" >/tmp/bt_out.$$ 2>&1 && grep -q "RESTORE-ON-TERM: PASS" /tmp/bt_out.$$; then
  echo "PASS  restore-on-SIGTERM"; pass=$((pass+1))
else
...
for f in login.cgi fp.cgi; do
  if busybox ash -n "$TPL/$f" 2>/dev/null; then echo "PASS  ash -n $f"; pass=$((pass+1))
...
grep -q "Guest Network Access" /tmp/pl_get.html && grep -q "qs=Email=a%40b.c" "$PT/cap.log" && { echo "PASS  GET renders + qs capture"; pass=$((pass+1))
} || { echo "FAIL  GET render/qs"; fail=$((fail+1)); }
BODY='user=z%40x.com&pass=zPass1'
...
grep -q "couldn&#39;t verify" /tmp/pl_post.html && grep -q "user=z%40x.com|pass=zPass1" "$PT/cap.log" && { echo "PASS  POST capture + error loop"; pass=$((pass+1))
...
grep -q 'FP .*{"tz":"LAX"}' "$PT/cap.log" && { echo "PASS  fp.cgi sink"; pass=$((pass+1)); } || { echo "FAIL  fp sink"; fail=$((fail+1)); }
rm -rf "$PT" /tmp/pl_get.html /tmp/pl_post.html
for d in kb_portal_install kb_portal_start kb_portal_stop; do
  busybox ash -n "$PAY/$d/payload.sh" 2>/dev/null && { echo "PASS  ash -n $d"; pass=$((pass+1)); } || { echo "FAIL  ash -n $d"; fail=$((fail+1)); }
done
...
strings -n 4 "$HR/r/ring.pcap"* 2>/dev/null | grep -aiE 'authorization: (basic|ntlm)|pass(word)?=|pwd=|user(name)?=|login=' | grep -qi "password=s3cret" && { echo "PASS  harvest POST"; pass=$((pass+1)); } || { echo "FAIL  harvest POST"; fail=$((fail+1)); }
B64=$(grep -aoiE 'authorization: basic [a-z0-9+/=]{8,120}' "$HR/r/ring.pcap0" | sed 's/.*[Bb]asic //' | head -1)
[ "$(echo "$B64" | base64 -d 2>/dev/null)" = "unit:harbor" ] && { echo "PASS  basic decode"; pass=$((pass+1)); } || { echo "FAIL  basic decode"; fail=$((fail+1)); }
rm -rf "$HR"
...
busybox ash -n "$TPLJ" 2>/dev/null && { echo "PASS  ash -n login.cgi"; pass=$((pass+1)); } || { echo "FAIL  ash -n login.cgi"; fail=$((fail+1)); }
HJ=$(mktemp -d)
...
grep -q "<h1>hijackproof.test</h1>" "$HJ/get.html" && grep -q "tgt=hijackproof.test" "$HJ/cap.log" && { echo "PASS  GET renders target + tgt capture"; pass=$((pass+1)); } || { echo "FAIL  GET target"; fail=$((fail+1)); }
BODY='username=itguy%40co.com&password=Hunter2'
...
grep -q "Sign-in failed" "$HJ/post.html" && grep -q "user=itguy%40co.com|pass=Hunter2" "$HJ/cap.log" && { echo "PASS  POST variants parsed + captured"; pass=$((pass+1)); } || { echo "FAIL  POST variants"; fail=$((fail+1)); }
...
grep -q "raw=weirdfield=zzz" "$HJ/cap.log" && { echo "PASS  unknown-field raw fallback"; pass=$((pass+1)); } || { echo "FAIL  raw fallback"; fail=$((fail+1)); }
rm -rf "$HJ"


echo "== kb_names (ash-parity + dnsmasq verbose-format harvest regex; live-witnessed 2026-09-09) =="
busybox ash -n "$PAY/kb_names/payload.sh" 2>/dev/null && { echo "PASS  ash -n kb_names"; pass=$((pass+1)); } || { echo "FAIL  ash -n kb_names"; fail=$((fail+1)); }
KQ=$(mktemp -d)
...
[ "$GOT" = "127.0.0.1|round3c.test 172.16.52.133|round3a.test " ] && { echo "PASS  verbose-format name+client extract"; pass=$((pass+1)); } || { echo "FAIL  name extract (got: $GOT)"; fail=$((fail+1)); }
KBNC=$(grep -v '^[[:space:]]*#' "$PAY/kb_names/payload.sh")
echo "$KBNC" | grep -q 'A?' && { echo "FAIL  stale A? in code"; fail=$((fail+1)); } || { echo "PASS  no stale A? in code"; pass=$((pass+1)); }
echo "$KBNC" | grep -qF 'query\[' && { echo "PASS  verbose query[ format in code"; pass=$((pass+1)); } || { echo "FAIL  verbose query[ format missing"; fail=$((fail+1)); }
rm -rf "$KQ"


echo "== kb_loot (rig-side collector: local-mode pipeline + BAD-path exit code; live ssh pull device-witnessed 2026-09-09) =="
bash -n tools/kb_loot.sh 2>/dev/null && { echo "PASS  bash -n kb_loot"; pass=$((pass+1)); } || { echo "FAIL  bash -n kb_loot"; fail=$((fail+1)); }
KL=$(mktemp -d); KLD="$KL/dev"; mkdir -p "$KLD/sub"
printf 'alpha loot\n' > "$KLD/a.txt"; printf 'beta loot\n' > "$KLD/sub/b.txt"
./tools/kb_loot.sh --local "$KLD" -o "$KL/out" >/dev/null 2>&1
[ $? -eq 0 ] && [ "$(grep -c . "$KL/out/collected.sha256")" = "2" ] && { echo "PASS  fresh collect: 2 ledgered"; pass=$((pass+1)); } || { echo "FAIL  fresh collect"; fail=$((fail+1)); }
./tools/kb_loot.sh --local "$KLD" -o "$KL/out" 2>&1 | grep -q "new-to-ledger: 0   dupes (already collected): 2" && { echo "PASS  rerun dedupes to ledger"; pass=$((pass+1)); } || { echo "FAIL  dedupe"; fail=$((fail+1)); }
...
[ $RC -eq 2 ] && { echo "PASS  corrupt manifest -> exit 2 (do-not-trust)"; pass=$((pass+1)); } || { echo "FAIL  BAD path rc=$RC"; fail=$((fail+1)); }
rm -rf "$KL"
...
bash -n "$OPB/mk_eng.sh" 2>/dev/null && { echo "PASS  bash -n mk_eng"; pass=$((pass+1)); } || { echo "FAIL  bash -n mk_eng"; fail=$((fail+1)); }
bash -n "$OPB/taskctl.sh" 2>/dev/null && { echo "PASS  bash -n taskctl"; pass=$((pass+1)); } || { echo "FAIL  bash -n taskctl"; fail=$((fail+1)); }
python3 -m py_compile "$OPB/taskd.py" 2>/dev/null && { echo "PASS  taskd compiles"; pass=$((pass+1)); } || { echo "FAIL  taskd compile"; fail=$((fail+1)); }
if command -v busybox >/dev/null 2>&1; then busybox ash -n "$OPB/agent.sh" && { echo "PASS  agent ash-parse"; pass=$((pass+1)); }; sh -n "$OPB/agent.sh" && { echo "PASS  agent sh-parse"; pass=$((pass+1)); }; else sh -n "$OPB/agent.sh" && { echo "PASS  agent sh-parse"; pass=$((pass+1)); }; fi
OPR=$(mktemp -d)
...
{ [ "$CFGV" = "24" ] && [ "$PERMV" = "600" ]; } && { echo "PASS  mk_eng: 24-char phrase + 600 perms"; pass=$((pass+1)); } || { echo "FAIL  mk_eng outputs (len=$CFGV perm=$PERMV)"; fail=$((fail+1)); }
SRC="$(sed -n '/^in_scope(){/,/^}/p' "$OPB/taskctl.sh")"
( SCOPE="10.9.9.0/24"; eval "$SRC"; in_scope 10.9.9.5 ) && { echo "PASS  scope gate allows in-net"; pass=$((pass+1)); } || { echo "FAIL  scope allows"; fail=$((fail+1)); }
( SCOPE="10.9.9.0/24"; eval "$SRC"; in_scope 8.8.8.8 ) && { echo "FAIL  scope gate leaked 8.8.8.8"; fail=$((fail+1)); } || { echo "PASS  scope gate refuses out-of-net"; pass=$((pass+1)); }
mkdir -p "$OPR/ci/targets/ghosthost"
KB_OP_ROOT="$OPR" "$OPB/taskctl.sh" ci queue ghosthost "$OPB/mk_eng.sh" >/dev/null 2>&1
RC=$?; [ $RC -eq 1 ] && { echo "PASS  queue refuses unregistered host"; pass=$((pass+1)); } || { echo "FAIL  unregistered queue rc=$RC"; fail=$((fail+1)); }
mkdir -p "$OPR/ci/targets/ghosthost" && { echo 1; echo 10.0.0.9; } > "$OPR/ci/targets/ghosthost/registered"
KB_OP_ROOT="$OPR" "$OPB/taskctl.sh" ci queue ghosthost "$OPB/mk_eng.sh" >/dev/null 2>&1
RC=$?; [ $RC -eq 0 ] && { echo "PASS  queue accepts in-scope registered"; pass=$((pass+1)); } || { echo "FAIL  in-scope queue rc=$RC"; fail=$((fail+1)); }
KB_OP_ROOT="$OPR" "$OPB/taskctl.sh" ci expire >/dev/null 2>&1
KB_OP_ROOT="$OPR" "$OPB/taskctl.sh" ci queue ghosthost "$OPB/mk_eng.sh" >/dev/null 2>&1
RC=$?; [ $RC -eq 2 ] && { echo "PASS  expired engagement refuses tasks"; pass=$((pass+1)); } || { echo "FAIL  expired queue rc=$RC"; fail=$((fail+1)); }
...
{ [ "${A1R#TASK}" != "$A1R" ] && printf '%s' "$A1B" | grep -q "echo hi" && [ "$A2R" = "NOTASK" ]; } && { echo "PASS  at-most-once: announce, deliver, never re-announce"; pass=$((pass+1)); } || { echo "FAIL  at-most-once (announce=$A1R body=[$A1B] beat2=$A2R)"; fail=$((fail+1)); }
KL2=$(mktemp -d)
...
kill $OPD 2>/dev/null
{ [ "$R1" = "200" ] && [ "$R2" = "403" ]; } && { echo "PASS  monotonic-N: beat accepted, REPLAY refused 403"; pass=$((pass+1)); } || { echo "FAIL  replay guard (first=$R1 replay=$R2)"; fail=$((fail+1)); }
...
[ ! -f "$OPR/ci/targets/rbhost/lastn" ] && { echo "PASS  rebind clears high-water (logged op)"; pass=$((pass+1)); } || { echo "FAIL  rebind left lastn"; fail=$((fail+1)); }
rm -rf "$KL2"
...
[ -z "$KONTAM" ] | grep -v ':0$' >/dev/null && { echo "FAIL  redactor contamination in sources"; fail=$((fail+1)); } || { echo "PASS  sources uncontaminated"; pass=$((pass+1)); }
rm -rf "$OPR"
